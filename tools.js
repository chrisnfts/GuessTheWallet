...
function toSeed (mnemonic) {
    passphrase = '';
    mnemonic = splitWords(mnemonic).join(" "); // removes duplicate blanks
    ...
    /*
    hmacSHA512 is a function 
    :function(key is the mnemonic (real passphrase)) {
...
sjcl.misc.pbkdf2 = function (password, salt, count, length, Prff) {
    ...
    // ^ password is mnemonic (passphrase the real one) abd salt is (word called "mnemonic" convertd to array!! and count is the bip39 word lest lenth which is 2048, length is 512 , and Prff )
  count = count || 1000
  ...
  // ^ Already convert string to array!! you dont need this!!!
  ...
  for (k = 1; 32 * out.length < (length || 1); k++) {
    ...
    // ONLY LOOP ONE TIME!!!
    // [k] is 1 !!
    // b.concat(salt,[k]) always is [1835951469,1869506915,1]
    // prf.encrypt : encrypto the passphrase
